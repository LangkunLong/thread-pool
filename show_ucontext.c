...
//	TBD();
	...
	printf("memory address of the program counter (RIP) saved "
	       "in mycontext = 0x%lx\n",
	       (unsigned long)mycontext.uc_mcontext.gregs[REG_RIP]); //return address
	 ...
	 * the main function. 
	 */ 
        //i think it is stored on the thread's stack along with local varaibles but unable to find any registers that contain it

	/* Now, think about the stack. */
	/* QUESTIONS: Are setcontext_called and err stored on the stack? 
         * Yes, they are in the local stack
	 * Does the stack grow up or down? What stack-related data is stored in
	 * mycontext.uc_mcontext.gregs[]? 
         * Stack grows down, the stack pointer points to the last value pushed to stack
	 */
	...
	printf("stack pointer register (RSP) stored in mycontext = 0x%lx\n",
        ...
        //here we are converting to integer already so no need to multiply by 4 at the end
...
show_interrupt(void)
{
	...
	/* QUESTION: how did we get here if there was an assert above? 
         * setcontext never returns if its successful
        
         we get here when we used setcontext to restore user context. But we set a flag to avoid running in a infinite loop.
         * We got here because of the flag because we already ran user context once. We probabaly want to exit now
         * 
	...
	* now think about interrupts. you will need to understand how they
	...
	interrupts_on();
        //initilizes a signal set and adds signals to it. Enabling interrupts means that we don't want to block these signals 
	...
	printf("interrupt is disabled = %d\n",
	       (unsigned int)sigismember(&mycontext.uc_sigmask, SIG_TYPE)); //should say true 

	interrupts_off(); //should see more signals being blocked 
	...
	printf("interrupt is disabled = %d\n",
	       (unsigned int)sigismember(&mycontext.uc_sigmask, SIG_TYPE)); //should say false
}
