...
interrupts_set(int enabled)
{
	...
	set_signal(&mask); //initilizes the signalset, and adds a signal to mask
	if (enabled) {
            //set of signals in mask set should not be blocked
		ret = sigprocmask(SIG_UNBLOCK, &mask, &omask);
	} else {
            //set of signals in mask set should be blocked; usually in critical sections of code
...
interrupts_enabled()
{
	...
	int ret;
        ...
        //since new set is null, old set returns current set of signals being blocked
	ret = sigprocmask(0, NULL, &mask);
	assert(!ret);
        //test if SIG_TYPE is within the signals being blocked 
